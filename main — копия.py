...
#fact_string_list
fact_list = [[c.v for c in row] for row in input_sheet.rows()]
#print(*fact_list[29:50], sep = '\n')

for i, row in enumerate(input_sheet.rows(sparse=True)):
    print(i)
    ...
    if Flag and string_list[7] == "Опер.план (ЦПП)":
        ...
        if set(str(string_list[2]).lower()) & set(ascii_lowercase):  # если есть буквы в Артикуле ДСЕ
            plan_dict_current_week["CODE"] = string_list[2]
        else:
            plan_dict_current_week['CODE'] = str(string_list[2]).split('.')[0].rjust(18, '0') # приводим Артикул в 18-значный формат
            #print(plan_dict['CODE'])
        if int(string_list[current_week_plan_column]) > 0:
            plan_current_week = int(string_list[current_week_plan_column])
            plan_dict_current_week["#PLAN"] = int(string_list[current_week_plan_column])
            plan_dict_current_week["AMOUNT"] = int(plan_dict_current_week["#PLAN"]) - int(plan_dict_current_week["#FACT"])
        ...
        while (
            ...
            if set(str(string_list[2]).lower()) & set(ascii_lowercase):  # если есть буквы в Артикуле ДСЕ
                ...
                plan_dict_next_week['CODE'] = str(string_list[2]).split('.')[0].rjust(18,'0') # приводим Артикул в 18-значный формат
            if int(string_list[current_week_plan_column+next_week_plan_column]) > 0:
                plan_dict_next_week["#PLAN"] = int(string_list[
                current_week_plan_column + next_week_plan_column
            ])
                plan_dict_next_week["AMOUNT"] = int(string_list[
                current_week_plan_column + next_week_plan_column
            ])
                #print(f'{counter}')
                counter += 1
